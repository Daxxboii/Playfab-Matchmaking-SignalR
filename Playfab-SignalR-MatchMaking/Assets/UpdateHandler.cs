...
public class UpdateHandler
{
    ...
    private UnityFileDownloader _downloader;

    public UnityFileDownloader Downloader
    {
        get
        {
            return _downloader;
    ...
    // list of resources to downlaod from CDN
    // and internal paths for resource files specified by Cloudscript
    private List<FileManifest> _fileManifests = new List<FileManifest>();

    public List<FileManifest> FileManifest
    {
        get
        {
            return _fileManifests;
        }
    }
    ...
    {
        foreach (var manifest in _fileManifests)
        {
    ...
    private void Reset(bool resetManifest)
    {
        ...
        {
            _downloader = null;
            _fileManifests = new List<FileManifest>();
        }
    ...
    private void OnUpdateCloudResult(ExecuteFunctionResult result)
    {
        ...
        {
            _fileManifests.Add(new FileManifest
            {
                Filename = json["content"][i]["filename"],
                Name = json["content"][i]["name"],
                PartialPath = json["content"][i]["contentKey"],
                Path = DataPath + Path.DirectorySeparatorChar + json["content"][i]["contentKey"]
            });
        }
        ...
        for (var i = 0; i < json["content"].Count; i++)
        {
            if (!File.Exists(FileManifest[i].Path))
            {
                ...
                string DownloadUrl = Utilities.GetUrlFromKey(config.CDN, json["content"][i]["contentKey"]);

                if (Utilities.ValidateUri(DownloadUrl)) FileManifest[i].URI = DownloadUrl;
                else
                {
                    if (IsUpdating)
                    {
    ...
    {
        if (_downloader != null || !IsUpdating || _fileManifests == null) return false;
        if (_fileManifests.Count == 0) return false;
        ...
        */
        ...
        foreach (var manifest in _fileManifests)
        {
            ...
            {
                missingFiles = true;
                if (_downloader == null) _downloader = new UnityFileDownloader();
                _downloader.PendingURIS.Add(manifest.URI);
        ...
        if (missingFiles)
        {
            ...
            _downloader.OnCancel += OnUpdateFailure;
            _downloader.OnDownloadsSuccess += OnUpdateSuccess;
